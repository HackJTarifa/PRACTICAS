...
class GameLogicC{
    ...
    playerWin(board, player){
        const romsLength = board.getRomsLength();
        const columnsLength = board.getColumnsLength();
        let cuantroEnRaya = false;
        for(let idFil = 0; idFil < romsLength ; idFil++){
            for(let idCol = 0; idCol < columnsLength; idCol++){
...
class Connecta4{
    ...
    initialValues(){
        ...
        this.gameLogic = new GameLogicC(this.turn);
...
class DataInputM{
    ...
    callEnterDataByFunction(playerType, secretCodeManager){
        function getCombinationPlayer(secretCodeManager){
            ...
            return new Combination(inputCombination);
        }
...
class SecretCodeManager{
    ...
    generateCombination(){
        ...
        do{
            ...
            if(!winCombination.contains(this.getColor(index))){
                winCombination.add(this.validColors.add(index));
            }
